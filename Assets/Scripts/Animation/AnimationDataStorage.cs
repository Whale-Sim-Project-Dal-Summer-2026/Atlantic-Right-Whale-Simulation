...
namespace AnimationDataStorageManager {
    ...
    public void SaveWhaleAnimationData(WhaleState[] whaleStates, string filePath) {
        ...
        using (FileStream stream = new FileStream(filePath, FileMode.Create))
        {
            using (BinaryWriter writer = new BinaryWriter(stream)) {
                ...
                writer.Write((int)whaleStates.Length); // Total number of frames
                ...
                for (int i =0; i < whaleStates.Length; i++) {
                    ...
                    if (state.MainBody.Length > 0) WriteBodyBlock(writer, state.MainBody);
            ...
            using (FileStream stream = new FileStream(filePath, FileMode.Open))
            {
                using (BinaryReader reader = new BinaryReader(stream)) {
                    ...
                    for (int i = 0; i < totalFrames; i++) {
                        ...
                        if (state.MainBody.Length > 0) ReadBodyBlock(reader, state.MainBody);
            ...
            using (FileStream stream = new FileStream(filePath, FileMode.Open)){
                using (BinaryReader reader = new BinaryReader(stream)) {
                    ...
                    if (frameIndex < 0 || frameIndex >= totalFrames)
                        ...
                        $"Frame {frameIndex} is out of range || file has {totalFrames} frames.");
    ...
    private void ReadBodyBlock(BinaryReader reader, Global_AnimationData[] data) {
        for (int i = 0; i < data.Length; i++) {
            data[i].Position.x = (float)reader.ReadSingle();
            data[i].Position.y = (float)reader.ReadSingle();
            data[i].Position.z = (float)reader.ReadSingle();
            data[i].Rotation.x = (float)reader.ReadSingle();
            data[i].Rotation.y = (float)reader.ReadSingle();
            data[i].Rotation.z = (float)reader.ReadSingle();
            data[i].Rotation.w = (float)reader.ReadSingle();
            data[i].Speed = (float)reader.ReadSingle();
        }
    }
